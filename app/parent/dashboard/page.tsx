...
export default function ParentDashboard() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`parent-progress-${selected}`)
      .on("postgres_changes", {
        ...
        setProgress(prev => {
          const existing = prev.find(p => p.subject === newProg.subject && p.child_id === newProg.child_id);
          if (existing) return prev.map(p => p.subject === newProg.subject && p.child_id === newProg.child_id ? newProg : p);
          return [...prev, newProg];
        });
      })
      .on("postgres_changes", {
        ...
        setProgress(prev => prev.map(p => p.subject === updatedProg.subject && p.child_id === updatedProg.child_id ? updatedProg : p));
      })
      .subscribe();
    ...
    <div className="screen-container">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 px-4 pt-12 pb-20">
        <div className="flex items-center justify-between mb-1">
          <div>
            ...
            <h1 className="font-display text-2xl font-black text-white">{user?.name || "Ba / Mẹ"} 👋</h1>
          </div>
          <button onClick={handleLogout} className="bg-white/20 rounded-2xl p-2.5 active:scale-95 transition-all">
      ...
      <div className="px-4 -mt-12 pb-4 space-y-4">
          ...
          {child && (
            <div className="bg-gradient-to-r from-slate-50 to-blue-50 rounded-2xl p-3">
              <div className="flex items-center gap-3">
                <span className="text-4xl">{AVATAR_EMOJI[child.avatar] || "🐱"}</span>
                <div>
                  <p className="font-display font-black text-lg text-slate-800">{child.name}</p>
                  <p className="text-slate-500 text-sm font-bold">Lớp {child.grade} • Mã: <span className="text-blue-600 font-extrabold">{child.code}</span></p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { icon: <Flame className="w-5 h-5" />, label: "Streak", value: `${maxStreak} ngày`, color: "from-orange-400 to-red-500" },
            { icon: <Trophy className="w-5 h-5" />, label: "Tổng XP", value: totalXp, color: "from-yellow-400 to-orange-500" },
            { icon: <Target className="w-5 h-5" />, label: "Chính xác", value: `${avgAccuracy}%`, color: "from-green-400 to-teal-500" },
          ].map((s, i) => (
            <div key={i} className={`bg-gradient-to-br ${s.color} rounded-3xl p-4 text-white`}>
        ...
        {childProgress.length > 0 && (
          <div className="card space-y-3">
            <h2 className="font-display font-black text-lg text-slate-800">📊 Tiến độ từng môn</h2>
            {childProgress.map(p => {
              const subjectMap: Record<string, { label: string; emoji: string; color: string }> = {
                math: { label: "Toán học", emoji: "🔢", color: "bg-blue-500" },
                viet: { label: "Tiếng Việt", emoji: "📖", color: "bg-orange-500" },
                eng: { label: "Tiếng Anh", emoji: "🌍", color: "bg-green-500" },
                science: { label: "Khoa học", emoji: "🔬", color: "bg-yellow-500" },
              };
              const s = subjectMap[p.subject] || { label: p.subject, emoji: "📚", color: "bg-slate-500" };
              ...
              return (
                <div key={p.subject}>
                  ...
                  <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden">
                    <div className={`h-full ${s.color} rounded-full transition-all duration-700`} style={{ width: `${acc}%` }} />
                  </div>
                  <p className="text-xs text-slate-400 font-bold mt-1">{p.correct_questions || 0}/{p.total_questions || 0} câu đúng</p>
                </div>
              );
            })}
          </div>
        ...
        {childProgress.length === 0 && selected && (
          <div className="card text-center py-8">
            ...
            <p className="font-extrabold text-slate-500">Con chưa học bài nào</p>
            <p className="text-slate-400 text-sm mt-1 font-semibold">Chia sẻ mã <span className="text-blue-600 font-black">{child?.code}</span> cho con nhé!</p>
          </div>
        )}
